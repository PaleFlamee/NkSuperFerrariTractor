...
`Timer2`用于产生PWM信号(两路同分辨率)  

### 2.运行逻辑
1. `setup()`函数
    1. 各项功能初始化
    2. 通过**2P拨码开关**判断并标记小车所处的题目`currentTitleIndex`  
2. `loop()`函数
    1. 通过软件定时器a检测小车运行进度，在需要时切换PID运行模式`currentOperation`，开启/关闭声光模块开关`Blink`  
    2. 通过软件定时器b调节声光模块
    3. 通过软件定时器c更新`imuData` `ltmData`  
3. `Timer1_ISR()`函数
    1. 执行PID
